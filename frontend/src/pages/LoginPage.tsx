      ...
      <div className="hidden lg:flex w-[44%] bg-deep-green flex-col justify-between p-12 relative overflow-hidden select-none">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl bg-mint-green/20 flex items-center justify-center ring-1 ring-mint-green/30">
              <Leaf className="w-5 h-5 text-mint-green" />
            </div>
            <div>
              <p className="text-white font-bold text-xl leading-tight">VedSanjeevani</p>
              <p className="text-mint-green text-[10px] font-semibold tracking-widest uppercase">AIOS</p>
            </div>
          </div>
        </div>
        ...
        <div className="relative z-10 space-y-4">
          <h2 className="text-[2.6rem] font-bold text-white leading-[1.15]">
            Ancient Wisdom,<br />
            <span className="text-mint-green">Modern</span><br />
            Intelligence.
          </h2>
          <p className="text-sage-green text-sm leading-relaxed max-w-xs">
            Unified AI Operations System for Ayurvedic marketing, content, and campaign management across every channel.
          </p>
          ...
          <div className="flex gap-3 pt-2">
            {[
              ...
              <div key={m.label} className="bg-forest-green/60 rounded-lg px-3 py-2 ring-1 ring-forest-green">
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-10 overflow-y-auto">
        <div className="w-full max-w-sm space-y-8">
          ...
          <div className="flex lg:hidden items-center gap-2.5">
            ...
            <span className="font-bold text-foreground">VedSanjeevani</span>
          </div>

          {/* Header */}
          <div>
            <h1 className="text-3xl font-bold text-foreground tracking-tight">Welcome back</h1>
            <p className="text-muted-foreground text-sm mt-1.5">Sign in to continue to your dashboard</p>
          </div>
          ...
          <div className="rounded-xl border border-border bg-card p-4 space-y-3">
            ...
            {tab === 'staff' ? (
              <button
                type="button"
                onClick={() => { setEmail('admin@vedsanjeevani.com'); setPassword('admin@123'); setError('') }}
              ...
              >
                <div>
                  ...
                  <p className="text-[11px] text-muted-foreground font-mono">admin@vedsanjeevani.com</p>
                </div>
              ...
              <div className="space-y-1.5">
                {AGENCY_CREDS.map(a => (
                  <button
                    key={a.agencyId}
                    type="button"
                    onClick={() => { setEmail(a.email); setPassword('agency@123'); setError('') }}
                    className="w-full flex items-center justify-between px-3 py-2 rounded-lg border border-border hover:bg-muted hover:border-primary/30 transition-colors cursor-pointer text-left group"
                  >
                    <div>
                      <p className="text-xs font-medium text-foreground">{a.agencyName}</p>
                      <p className="text-[11px] text-muted-foreground font-mono">{a.email}</p>
                    </div>
                    <span className="text-[10px] text-muted-foreground group-hover:text-primary transition-colors">Click to fill →</span>
                  </button>
                ))}
