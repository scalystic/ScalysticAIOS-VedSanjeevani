...
import {
  ...
  Sparkles, AlertTriangle, Star, Zap, AlertCircle,
} from 'lucide-react'
...
import type { CampaignObjective, CampaignItem, ContentCategory } from '@/context/AppData'
...
type DatePreset = 'this_week' | 'this_month' | 'this_year' | 'last_7d' | 'last_30d' | 'last_90d' | 'year_2025' | 'custom'
...
const ALL_DAILY: DayData[] = Array.from({ length: TOTAL_DAYS }, (_, i) => {
  ...
  const spend     = Math.round(12_000 * (1 + wave) * (isWeekend ? 1.22 : 1) * growth)
...
function getPresetRange(preset: DatePreset, customStart: string, customEnd: string): [string, string] {
...
function sumPeriod(data: DayData[], platform: Platform) {
  ...
  const impressions = Math.round(spend * 7.7)
...
function exportCSV(campaigns: CampaignItem[], start: string, end: string) {
  const headers = [
    'Campaign Name','Platform','Objective','Status','Agency',
    'Start Date','End Date','Budget Type','Budget (₹)',
    'Spend (₹)','Revenue (₹)','ROAS','Impressions','Clicks','CTR (%)','Conversions','Cost/Conv (₹)',
  ]
  const rows = campaigns.map(c => [
    `"${c.name}"`, c.platform, c.objective, c.status, c.agency ?? 'Unassigned',
    c.startDate, c.endDate ?? 'Ongoing', c.budgetType, c.budget,
    c.spend, Math.round(c.revenue), c.roas.toFixed(2),
    c.impressions, c.clicks, c.ctr.toFixed(2), c.conversions,
    ...
    <div className="bg-card border border-border rounded-lg shadow-lg px-3 py-2.5 text-xs min-w-[140px]">
      ...
      {payload.map((e) => (
        <div key={e.name} className="flex items-center gap-2 mb-1 last:mb-0">
          {(e.color ?? e.fill) && (
            <span className="h-2 w-2 rounded-full shrink-0" style={{ backgroundColor: e.color ?? e.fill }} />
          )}
...
function KpiCard({ label, value, change, positiveIsGood = true }: {
  label: string; value: string; change: number | null; positiveIsGood?: boolean
}) {
      ...
      <CardContent className="px-5 pt-5 pb-4">
        ...
        {change !== null ? (
          <div className={cn(
            ...
            good ? 'text-success' : isNegative === isPositive ? 'text-gray' : 'text-error'
          )}>
...
function LeaderboardRow({ rank, name, sub, spend, revenue, roas, maxRoas, impressions, conversions, sortMetric }: {
  rank: number; name: string; sub?: string
  spend: number; revenue: number; roas: number; maxRoas: number
  impressions: number; conversions: number; sortMetric: SortMetric
}) {
  const barPct   = maxRoas > 0 ? Math.min((roas / maxRoas) * 100, 100) : 0
  const barColor = roas >= 4 ? C.success : roas >= 3 ? C.warning : roas > 0 ? C.error : C.lightGray
  ...
  return (
    <div className="flex items-center gap-3 px-5 py-3.5 border-b border-border last:border-0 hover:bg-off-white transition-colors">
      <span className="w-6 text-sm font-semibold text-gray text-right shrink-0">{rank}</span>
      ...
      <div className="hidden sm:block w-24 text-right">
        <p className="text-sm font-semibold text-foreground tabular-nums">
          {sortMetric === 'spend'       ? fmtINR(spend)           :
           sortMetric === 'revenue'     ? fmtINR(revenue)         :
           sortMetric === 'impressions' ? fmtCompact(impressions) : ''}
        </p>
      ...
      <div className="flex items-center gap-2 w-36 shrink-0">
        ...
        <span className={cn('text-sm font-bold tabular-nums w-14 text-right',
          roas >= 4 ? 'text-success' : roas >= 3 ? 'text-warning' : roas > 0 ? 'text-error' : 'text-gray'
        )}>
          {fmtRoas(roas)}
        </span>
      </div>
      <div className="hidden lg:block w-20 text-right">
        <p className="text-sm text-slate tabular-nums">{fmtCompact(conversions)}</p>
  ...
  return (
    <div
      ...
      style={{
        ...
        background: 'linear-gradient(148deg, rgba(15,163,142,0.045) 0%, transparent 52%)',
      }}
    >
      {/* Header */}
      ...
      {loading ? (
        ...
        <div className="bg-card rounded-xl border border-border px-6 py-10 text-center">
          ...
          <p className="text-sm text-gray">No strong signals in the current filters.</p>
          <p className="text-xs text-gray mt-1">Try a wider date range or remove dimension filters.</p>
        </div>
      ) : (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
...
export default function AnalyticsPage() {
  ...
  }, [rangeStart, rangeEnd, platform, agencyFilter, creatorFilter, typeFilter, categoryFilter])

  // ── Daily trend data ──
  const currDays = useMemo(() => sliceDailyData(rangeStart, rangeEnd), [rangeStart, rangeEnd])
  const prevDays = useMemo(() => sliceDailyData(prevStart,  prevEnd),  [prevStart,  prevEnd])

  const curr = useMemo(() => sumPeriod(currDays, platform), [currDays, platform])
  const prev = useMemo(() => sumPeriod(prevDays, platform), [prevDays, platform])
  ...
  const activeCampaignIds = useMemo(() => new Set(filteredCampaigns.map(c => c.id)), [filteredCampaigns])
  ...
  const creators    = useMemo(() => [...new Set(contentItems.map(c => c.creator))].sort(), [contentItems])
  ...
  const contentLeader = useMemo(() => {
    const map = new Map<string, { name: string; type: string; creator: string; spend: number; revenue: number; impressions: number; conversions: number }>()
    for (const ad of filteredAdItems) {
      ...
      const prev = map.get(ad.contentId) ?? { name: c.name, type: c.type, creator: c.creator, spend: 0, revenue: 0, impressions: 0, conversions: 0 }
  ...
  const agencyLeader = useMemo(() => {
    const map = new Map<string, { name: string; type: string; spend: number; revenue: number; impressions: number; conversions: number }>()
    for (const c of filteredCampaigns) {
      if (!c.agency) continue
      const agency = agencyItems.find(a => a.name === c.agency)
      const prev = map.get(c.agency) ?? { name: c.agency, type: agency?.type ?? 'performance', spend: 0, revenue: 0, impressions: 0, conversions: 0 }
      map.set(c.agency, {
        ...prev,
  ...
  const creatorLeader = useMemo(() => {
    const map        = new Map<string, { spend: number; revenue: number; impressions: number; conversions: number; contentCount: number }>()
    ...
    for (const ad of filteredAdItems) {
      ...
      const prev    = map.get(creator) ?? { spend: 0, revenue: 0, impressions: 0, conversions: 0, contentCount: 0 }
  ...
  const typeLeader = useMemo(() => {
    const map = new Map<string, { spend: number; revenue: number; impressions: number; conversions: number; adCount: number }>()
    for (const ad of filteredAdItems) {
      const t = ad.contentType
      if (typeFilter !== 'all' && t !== typeFilter) continue
      const prev = map.get(t) ?? { spend: 0, revenue: 0, impressions: 0, conversions: 0, adCount: 0 }
      map.set(t, {
        spend:       prev.spend       + ad.spend,
        revenue:     prev.revenue     + ad.spend * ad.roas,
        impressions: prev.impressions + ad.impressions,
  ...
  const formatLeader = useMemo(() => {
    const map = new Map<ContentCategory, { spend: number; revenue: number; impressions: number; conversions: number; adCount: number }>()
    for (const ad of filteredAdItems) {
      if (!ad.contentId) continue
      const c = contentItems.find(x => x.id === ad.contentId)
      if (!c) continue
      ...
      const prev = map.get(cat) ?? { spend: 0, revenue: 0, impressions: 0, conversions: 0, adCount: 0 }
      map.set(cat, {
        spend:       prev.spend       + ad.spend,
        revenue:     prev.revenue     + ad.spend * ad.roas,
        impressions: prev.impressions + ad.impressions,
    ...
    return [...map.entries()].map(([cat, v]) => ({
      name: CATEGORY_LABEL[cat], rawCategory: cat, ...v,
    }))
  }, [filteredAdItems, contentItems, categoryFilter])

  // ── Sort leaderboard rows ──
  function sortRows<T extends { spend: number; revenue: number; impressions: number; conversions: number }>(rows: T[]) {
    const key = sortMetric === 'roas'
      ? (r: T) => r.spend > 0 ? r.revenue / r.spend : 0
      : (r: T) => r[sortMetric]
    return [...rows].sort((a, b) => key(b) - key(a))
  }

  const contentSorted = useMemo(() => sortRows(contentLeader), [contentLeader, sortMetric])
  const agencySorted  = useMemo(() => sortRows(agencyLeader),  [agencyLeader,  sortMetric])
  const creatorSorted = useMemo(() => sortRows(creatorLeader), [creatorLeader, sortMetric])
  const typeSorted    = useMemo(() => sortRows(typeLeader),    [typeLeader,    sortMetric])
  const formatSorted  = useMemo(() => sortRows(formatLeader),  [formatLeader,  sortMetric])

  function maxRoasOf<T extends { spend: number; revenue: number }>(rows: T[]) {
    return Math.max(...rows.map(r => r.spend > 0 ? r.revenue / r.spend : 0), 0.1)
  }

  // ── Platform split for pie chart ──
  const platformPie = useMemo(() => {
    const meta   = filteredCampaigns.filter(c => c.platform === 'meta').reduce((s, c) => s + c.spend, 0)
    const google = filteredCampaigns.filter(c => c.platform === 'google').reduce((s, c) => s + c.spend, 0)
    return [
  ...
  const insights = useMemo((): Insight[] => {
    ...
    // ── Signal 2: Top content asset ──
    if (contentSorted.length > 0) {
      const top     = contentSorted[0]
      const topRoas = top.spend > 0 ? top.revenue / top.spend : 0
      const avgRoas = curr.roas
      const premium = avgRoas > 0 ? Math.round((topRoas / avgRoas - 1) * 100) : 0
      const share   = curr.spend > 0 ? Math.round((top.spend / curr.spend) * 100) : 0
      const creator = (top as { creator?: string }).creator ?? 'your team'
      const name    = top.name.length > 26 ? top.name.slice(0, 23) + '…' : top.name
    ...
    if (currDays.length >= 6) {
      ...
      const fhRoas  = fhSpend > 0 ? fhDays.reduce((s, d) => s + d.revenue, 0) / fhSpend : 0
      const shRoas  = shSpend > 0 ? shDays.reduce((s, d) => s + d.revenue, 0) / shSpend : 0
    ...
    if (currDays.length >= 5) {
      const avgSpend = currDays.reduce((s, d) => s + d.spend, 0) / currDays.length
      const peakDay  = currDays.reduce((m, d) => d.spend > m.spend ? d : m, currDays[0])
      const spikePct = avgSpend > 0 ? (peakDay.spend - avgSpend) / avgSpend * 100 : 0

      if (spikePct > 35) {
        const peakRoas = peakDay.spend > 0 ? peakDay.revenue / peakDay.spend : 0
        out.push({
          id:          'spend-anomaly',
          type:        'anomaly',
    ...
    if (currDays.length >= 7 && curr.revenue > 0) {
      ...
      const trendF   = (() => {
        ...
        const mid  = Math.floor(currDays.length / 2)
        const fhR  = currDays.slice(0, mid).reduce((s, d) => s + d.revenue, 0) / mid
        const shR  = currDays.slice(mid).reduce((s, d) => s + d.revenue, 0) / (currDays.length - mid)
  ...
  }, [filteredCampaigns, contentSorted, formatSorted, currDays, curr, platform])

  // ── Trend chart x-axis interval ──
  const xInterval = currDays.length <= 14 ? 0 : currDays.length <= 60 ? 4 : currDays.length <= 130 ? 13 : 29

  const tip = (fmt: (v: number) => string) => (props: TTooltip) => <ChartTooltip {...props} fmt={fmt} />

  // ── Active leaderboard rows ──
  const activeRows = (
    leaderTab === 'content' ? contentSorted :
    leaderTab === 'agency'  ? agencySorted  :
    leaderTab === 'creator' ? creatorSorted :
    leaderTab === 'format'  ? formatSorted  :
    typeSorted
  )
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="mt-0.5 text-sm text-gray">Cross-channel performance — {periodLabel}</p>
        </div>
        <Button
          variant="outline"
          size="sm"
      ...
      <div className="space-y-3">
          ...
          {PRESETS.map(p => (
            <button
              ...
              className={cn(
                ...
                preset === p.value
                  ? 'bg-deep-green text-off-white border-deep-green'
                  : 'bg-card text-gray border-border hover:text-foreground hover:border-foreground'
              )}
            >
              {p.label}
            </button>
        ...
        {preset === 'custom' && (
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-2">
              ...
              <Input type="date" value={customStart} onChange={e => setCustomStart(e.target.value)} className="w-40 h-8 text-sm" />
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xs text-gray">To</span>
              <Input type="date" value={customEnd}  onChange={e => setCustomEnd(e.target.value)}   className="w-40 h-8 text-sm" />
            </div>
          </div>
        )}
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Select value={platform} onValueChange={v => setPlatform(v as Platform)}>
            <SelectTrigger className="w-[136px] h-8 text-sm"><SelectValue /></SelectTrigger>
            <SelectContent>
          ...
          <Select value={agencyFilter} onValueChange={setAgencyFilter}>
            <SelectTrigger className="w-[158px] h-8 text-sm"><SelectValue /></SelectTrigger>
            <SelectContent>
          ...
          <Select value={creatorFilter} onValueChange={setCreatorFilter}>
            <SelectTrigger className="w-[148px] h-8 text-sm"><SelectValue /></SelectTrigger>
            <SelectContent>
          ...
          <span className="ml-auto text-xs text-gray">
            {filteredCampaigns.length} campaign{filteredCampaigns.length !== 1 ? 's' : ''} in range
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 xl:grid-cols-6">
        <KpiCard label="Total Spend"    value={fmtINR(curr.spend)}           change={pctChange(curr.spend,       prev.spend)}       />
        <KpiCard label="Total Revenue"  value={fmtINR(curr.revenue)}         change={pctChange(curr.revenue,     prev.revenue)}     />
        <KpiCard label="Overall ROAS"   value={fmtRoas(curr.roas)}           change={pctChange(curr.roas,        prev.roas)}        />
        <KpiCard label="Impressions"    value={fmtCompact(curr.impressions)} change={pctChange(curr.impressions, prev.impressions)} />
        <KpiCard label="Clicks"         value={fmtCompact(curr.clicks)}      change={pctChange(curr.clicks,      prev.clicks)}      />
        <KpiCard label="Conversions"    value={fmtCompact(curr.conversions)} change={pctChange(curr.conversions, prev.conversions)} />
      </div>
      ...
      >
        {currDays.length === 0 ? (
          <p className="py-16 text-center text-sm text-gray">No data for the selected date range.</p>
        ) : (
          <>
            <ResponsiveContainer width="100%" height={260}>
              <AreaChart data={trendData} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                ...
                <XAxis dataKey="label" tick={{ fontSize: 11, fill: C.gray }} axisLine={false} tickLine={false} interval={xInterval} />
                <YAxis tick={{ fontSize: 11, fill: C.gray }} axisLine={false} tickLine={false} tickFormatter={fmtINR} width={54} />
                <RechartTooltip content={tip(fmtINR)} />
                <Area type="monotone" dataKey="Spend"   name="Spend"   stroke={C.teal} strokeWidth={2} fill="url(#gS)" dot={false} />
                <Area type="monotone" dataKey="Revenue" name="Revenue" stroke={C.mint} strokeWidth={2} fill="url(#gR)" dot={false} />
              </AreaChart>
            </ResponsiveContainer>
            <div className="flex items-center justify-center gap-5 mt-3">
              <span className="flex items-center gap-1.5 text-xs text-gray"><span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: C.teal }} />Spend</span>
              <span className="flex items-center gap-1.5 text-xs text-gray"><span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: C.mint }} />Revenue</span>
            </div>
          </>
        )}
      </ChartCard>
      ...
      <Card>
        <div className="flex items-center justify-between border-b border-border px-5">
          <div className="flex">
            {([ ['content','Content'], ['agency','Agency'], ['creator','Creator'], ['type','Media Type'], ['format','Content Format'] ] as [LeaderTab, string][]).map(([val, label]) => (
              <button
                key={val}
                onClick={() => setLeaderTab(val)}
                className={cn(
                  'px-4 py-3 text-sm font-medium border-b-2 transition-colors cursor-pointer',
                  leaderTab === val ? 'border-teal text-teal' : 'border-transparent text-gray hover:text-foreground'
                )}
              >
                {label}
              </button>
        ...
        <div className="flex items-center gap-3 px-5 py-2 border-b border-border bg-off-white">
          ...
          <span className="flex-1 text-xs font-semibold text-gray uppercase tracking-wide">Name</span>
          <span className="hidden sm:block w-24 text-right text-xs font-semibold text-gray uppercase tracking-wide capitalize">{sortMetric}</span>
          <span className="w-36 text-xs font-semibold text-gray uppercase tracking-wide">ROAS</span>
          <span className="hidden lg:block w-20 text-right text-xs font-semibold text-gray uppercase tracking-wide">Conv.</span>
        </div>

        {activeRows.length === 0 ? (
          <p className="py-12 text-center text-sm text-gray">No data for the current filters.</p>
        ) : (
          activeRows.map((row, i) => {
            const roas = row.spend > 0 ? row.revenue / row.spend : 0
            const sub  =
              leaderTab === 'content' ? (row as { creator?: string }).creator :
              leaderTab === 'agency'  ? ((row as { type?: string }).type === 'performance' ? 'Performance Marketing' : 'Creator Agency') :
              leaderTab === 'creator' ? `${(row as { contentCount?: number }).contentCount ?? 0} content piece${(row as { contentCount?: number }).contentCount !== 1 ? 's' : ''}` :
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <ChartCard title="Spend by Platform" description="Budget allocation across ad platforms">
          {platformPie.length === 0 ? (
            <p className="py-12 text-center text-sm text-gray">No campaign data for this period.</p>
          ) : (
              ...
              <ResponsiveContainer width="55%" height={220}>
                <PieChart>
                  <Pie
                    ...
                    strokeWidth={0}
                  >
                    {platformPie.map(e => <Cell key={e.name} fill={e.color} />)}
                  </Pie>
                  <RechartTooltip content={tip(fmtINR)} />
                </PieChart>
              </ResponsiveContainer>
                ...
                {platformPie.map(e => {
                  ...
                  return (
                    <div key={e.name}>
                      ...
                      <p className="text-xl font-bold text-foreground pl-4">{fmtINR(e.value)}</p>
                      <p className="text-xs text-gray pl-4">{Math.round((e.value / total) * 100)}% of spend</p>
                    </div>
                  )
                })}
              </div>
        ...
        <ChartCard title="Spend by Objective" description="How budget is distributed across campaign goals">
          {objectiveData.length === 0 ? (
            <p className="py-12 text-center text-sm text-gray">No campaign data for this period.</p>
          ) : (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart layout="vertical" data={objectiveData} margin={{ top: 0, right: 16, left: 0, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke={C.lightGray} horizontal={false} />
                <XAxis type="number" tick={{ fontSize: 11, fill: C.gray }} axisLine={false} tickLine={false} tickFormatter={fmtINR} />
                <YAxis type="category" dataKey="name" tick={{ fontSize: 11, fill: C.gray }} axisLine={false} tickLine={false} width={90} />
                <RechartTooltip content={tip(fmtINR)} />
                <Bar dataKey="Spend" name="Spend" fill={C.teal} radius={[0, 4, 4, 0]} maxBarSize={24} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </ChartCard>
