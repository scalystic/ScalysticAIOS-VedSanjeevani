  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto overflow-x-hidden px-2 py-4 space-y-0.5">
        {navigation.map((item) =>
          item.children ? (
            <SectionGroup key={item.id} item={item} collapsed={sidebarCollapsed} onExpand={() => useUIStore.getState().setSidebarCollapsed(false)} />
          ) : (
            <StandaloneLink key={item.id} item={item} collapsed={sidebarCollapsed} />
          )
        )}
      </nav>
  ...
  return (
    <div
      className={cn(
        ...
        collapsed ? 'justify-center flex-col gap-1 py-2 h-auto pt-4 pb-2' : 'justify-between'
      )}
    >
      {/* Logo */}
      <div className={cn('flex items-center gap-2.5', collapsed && 'justify-center')}>
        <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-mint-green text-deep-green font-bold text-[11px] tracking-tight shrink-0 select-none">
          VS
        </span>
        {!collapsed && (
...
function StandaloneLink({ item, collapsed }: { item: NavItem; collapsed: boolean }) {
  ...
  if (collapsed) {
    ...
    return (
      <Tooltip label={item.label} collapsed>
        <button
          type="button"
          onClick={() => { onExpand(); navigate(firstPath) }}
          className={cn(
      ...
      >
        ...
        {!collapsed && (
          <div className="min-w-0">
            <p className="text-xs font-semibold text-foreground truncate leading-tight">Admin User</p>
            <p className="text-[10px] text-gray truncate leading-tight">Admin Manager</p>
          </div>
        )}
      </div>
